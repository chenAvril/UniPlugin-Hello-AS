...
import com.chen.recordscreen.utils.FileUtil;
...
public class RecordService extends Service implements Handler.Callback {
     ...
     * 开始录屏
     */
    public void startRecord(boolean isAudio,double quality) {
        if (mIsRunning) {
            return;
        }

        setUpMediaRecorder(isAudio,quality);
    ...
    public void stopRecord() {
        ...
        if (listener != null){
            listener.onStopRecord(mRecordFilePath);
        }
...
//        }
    ...
    private void setUpMediaRecorder(boolean isAudio,double quality) {
        ...
        if(FileUtil.createOrExistsDir(new File(dirPath))){
            mRecordFilePath = dirPath +File.separator + setFileName() + ".mp4";
            ...
            //根据屏幕分辨率设置录制尺寸
            int width = (int) (mRecordWidth*quality);
            int height = (int) (mRecordHeight*quality);
            ...
            int bitRate = (int) ((mRecordWidth * mRecordHeight)*quality);
            mMediaRecorder.setVideoEncodingBitRate(bitRate);

            //mMediaRecorder.setOrientationHint(0);
//            CamcorderProfile profile = CamcorderProfile.get(CamcorderProfile.QUALITY_480P);
//            profile.videoFrameWidth = mRecordWidth;
//            profile.videoFrameHeight = mRecordHeight;
//            mMediaRecorder.setProfile(profile);
//            WXLogUtils.e("profile.fileFormat===>"+profile.fileFormat);
//            WXLogUtils.e("profile.videoFrameRate===>"+profile.videoFrameRate);
//            WXLogUtils.e("profile.videoFrameWidth===>"+profile.videoFrameWidth+"profile.videoFrameHeight===>"+profile.videoFrameHeight);
//            WXLogUtils.e("profile.videoBitRate===>"+profile.videoBitRate);
//            WXLogUtils.e("profile.videoCodec===>"+profile.videoCodec);
//            WXLogUtils.e("profile.quality===>"+profile.quality);
            try {
            ...
            mVirtualDisplay = mMediaProjection.createVirtualDisplay("MainScreen", width, height, mScreenDpi,
    ...
    public boolean handleMessage(Message msg) {
        switch (msg.what){
            case MSG_TYPE_COUNT_DOWN:{
                ...
                if (mRecordSeconds < 60 * 60 * 2){
                    mHandler.sendEmptyMessageDelayed(MSG_TYPE_COUNT_DOWN,1000);
                } else if (mRecordSeconds == 60 * 60 * 2){
